function(ian_define_component_impl name)
    ...
    target_sources(
        ...
        BASE_DIRS include
        FILES include/ian/${name}.hpp)

    target_compile_features(ian_${name} INTERFACE cxx_std_23)

    set_target_properties(ian_${name} PROPERTIES
        EXPORT_NAME ${name}
    ...
    target_link_libraries(ian_${name} INTERFACE
        dbg_macro
    ...
    if(IAN_BUILD_TESTS)
        ...
        target_link_libraries(ian_${name}_test
            PRIVATE
                ian::${name}
                ian_warnings
                ian_options
    ...
    if(IAN_BUILD_APPS)
        ...
        target_link_libraries(ian_${name}_app
            PRIVATE
                ian::${name}
                ian_warnings
                ian_options)
    ...
    if(NOT CMAKE_SKIP_INSTALL_RULES)
        ...
        install(
            FILES ${CMAKE_CURRENT_BINARY_DIR}/ian-${name}-config.cmake
                  ${CMAKE_CURRENT_BINARY_DIR}/ian-${name}-config-version.cmake
            DESTINATION ${CMAKE_INSTALL_DATADIR}/ian-${name}/cmake)
    endif()
endfunction()
